...
sudo apt-add-repository "deb http://apt.llvm.org/focal/ llvm-toolchain-focal-9 main" -y 
sudo apt-get update 
sudo apt-get install -y clang-9 --assume-yes
sudo apt-get install gsl-bin libgsl-dev --assume-yes
